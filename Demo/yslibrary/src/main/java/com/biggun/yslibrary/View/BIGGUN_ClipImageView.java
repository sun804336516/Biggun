...
public class BIGGUN_ClipImageView extends ImageView implements ViewTreeObserver.OnGlobalLayoutListener
{
    ...
    public void onGlobalLayout()
    {
        if (first) {
            ...
            float hei = getHeight();
            float scale = 1.0f;
            if (drawHei >= hei && drawWid >= wid) {
                scale = Math.max(drawHei / hei, drawWid / wid);
            }
            if (drawHei < hei && drawWid < wid) {
                scale = Math.min(hei / drawHei, wid / drawWid);
            }
            if (drawWid < wid && drawHei > hei) {
                scale = drawHei / hei;
            }
            if (drawWid > wid && drawHei < hei) {
                scale = drawWid / wid;
            }
            realRect.set((int) (wid - drawWid * scale) >> 1, (int) (hei - drawHei * scale) >> 1, (int) (wid + drawWid * scale) >> 1, (int) (hei + drawHei * scale) >> 1);
            mMatrix.postScale(scale, scale, (wid - drawWid) / 2, (hei - drawHei) / 2);
            setImageMatrix(mMatrix);
            Bitmap bitmap = Bitmap.createBitmap((int) (d.getIntrinsicWidth() * scale), (int) (d.getIntrinsicHeight() * scale), d.getOpacity()
            ...
            if (clipType == TYPE_RECT) {
                mRect = new Rect((int) (wid - drawWid) >> 1, (int) (hei - drawHei) >> 1, (int) (wid + drawWid) >> 1, (int) (hei + drawHei) >> 1);
            } else {
                if (clipType == TYPE_CIRCLE) {
                    int cirwid = (int) Math.min(drawWid, drawHei);
                    mRect = new Rect((int) (wid - cirwid) >> 1, (int) (hei - cirwid) >> 1
                            , (int) (wid + cirwid) >> 1, (int) (hei + cirwid) >> 1);
                }
            }
            first = false;
        }
    ...
    private void resetRect(int dx, int dy)
    {
        ...
        if (left < realRect.left) {
            left = realRect.left;
            right = mRect.right;
        }
        if (right > realRect.right) {
